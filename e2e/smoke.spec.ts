...
test.describe("smoke", () => {
  test("login page loads", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /登录|Meridian Plan/i })).toBeVisible();
  });

  test("demo user can login and reach home", async ({ page }) => {
    await page.goto("/login");
    await page.getByLabel(/邮箱/i).fill("demo@meridian.local");
  ...
  test("admin login reaches user management", async ({ page }) => {
    ...
    await page.getByLabel(/邮箱/i).fill("admin@meridian.local");
    await page.getByLabel(/密码/i).fill("password123");
    await page.getByRole("button", { name: /登录/i }).click();
