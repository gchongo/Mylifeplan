...
describe("i18n", () => {
  ...
  it("localizes feed action phrases", async () => {
    const messages = await loadMessages("en-US");
    const t = createTranslator("en-US", messages);
  ...
  it("localizes gantt schedule columns", async () => {
    const messages = await loadMessages("en-US");
    const t = createTranslator("en-US", messages);
